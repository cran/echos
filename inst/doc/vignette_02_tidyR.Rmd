...
The function `ESN()` is used in combination with `model()` from the `fabletools` package to train an Echo State Network for the variable `value`. The trained models are stored as a `mable` (i.e., *model table*). Additionally, an `ARIMA()` model is trained as a benchmark.
...
Forecasts are generated via the function `forecast()`, where the forecast horizon is set to `h = 18` (i.e., 18-month-ahead forecasts). The forecasts are stored as a `fable` (i.e., *forecast table*) and visualized along the historical training data.
